...
export default function Reports() {
    ...
    // Demo: cargar clientes de ejemplo solo cuando se abre el modal
    useEffect(() => {
        if (!showModal) return;
        setIsLoading(true);
        setTimeout(() => {
    ...
    // Demo: cargar reportes de ejemplo solo una vez al montar
    useEffect(() => {
        setReportes([
            { clientes: [{ id: '1', nombre: 'Cliente Demo 1' }], pdfName: 'reporte_demo.pdf', pdfUrl: undefined },
        ]);
    }, []);
    ...
    // Demo: Generar y descargar PDF localmente, actualizar cards en memoria
    const handleGenerateReport = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setReportes(prev => [{ clientes: selected, pdfName: `reporte_${Date.now()}.pdf` }, ...prev]);
    ...
    // Demo: Eliminar reporte solo en memoria
    const handleDeleteReport = (pdfName?: string) => {
        if (!pdfName) return;
